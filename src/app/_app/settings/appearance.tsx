...
function AppearanceSection() {
	...
	const fontSizeLabel = fontSize[0] <= 13 ? "صغير" : fontSize[0] <= 16 ? "متوسط" : fontSize[0] <= 19 ? "كبير" : "كبير جداً";
		...
		<SectionWrapper title="المظهر" description="خصّص شكل ومظهر مكتبتي ليناسب تفضيلاتك.">
					...
					{themeOptions.map((opt) => {
						...
						return (
							<div key={opt.value}>
								...
								<Label
									htmlFor={opt.value}
									className="flex cursor-pointer flex-col items-center gap-2 rounded-xl border-2 bg-muted/30 p-4 transition-all duration-150 hover:border-primary/40 peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/5"
								>
									<div className="flex h-10 w-10 items-center justify-center rounded-lg bg-muted peer-data-[state=checked]:bg-primary/10">
										<Icon className="h-5 w-5 text-foreground" weight="fill" />
									</div>
				...
				<div className="space-y-5">
					...
					<div className="space-y-3">
						<div className="flex items-center justify-between text-xs text-muted-foreground">
							...
							<span className="font-medium text-foreground">{fontSize[0]} بكسل</span>
						...
						return (
							<div key={opt.value}>
								...
								<Label
									...
									className="flex cursor-pointer items-center gap-3 rounded-xl border-2 bg-muted/30 px-4 py-3 transition-all duration-150 hover:border-primary/40 peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/5"
								>
									<Icon className="h-6 w-6 shrink-0 text-muted-foreground peer-data-[state=checked]:text-primary" weight="fill" />
									<div>
										<p className="text-sm font-semibold text-foreground">
											{opt.label}
										</p>
			...
			>
				<div className="divide-y divide-border/40">
					{[
						{ label: "الوضع المضغوط", desc: "استخدام مساحة أقل بين العناصر", defaultChecked: false },
						{ label: "عرض أغلفة الكتب", desc: "عرض صور الغلاف في القوائم", defaultChecked: true },
						{ label: "تحريك الانتقالات", desc: "تفعيل الحركات السلسة والمصقولة", defaultChecked: true },
						{ label: "تقليل الحركة", desc: "لإمكانية الوصول وتقليل الإرهاق البصري", defaultChecked: false },
					].map((item) => (
						<SettingRow
							key={item.label}
							label={item.label}
