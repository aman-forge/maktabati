		...
		>
				...
				<div className="grid gap-4">
					...
					<div className="grid gap-1.5">
						...
						<p className="text-[11px] text-muted-foreground">
							رابطك الفريد: مكتبتي.app/{username}
						</p>
					</div>
					...
					<div className="grid gap-1.5">
						<div className="flex items-center justify-between">
							...
							<span className={`text-[11px] ${bio.length > 280 ? "text-destructive" : "text-muted-foreground"}`}>
								{bio.length}/300
							</span>
						</div>
						<Textarea
					...
					<div className="grid gap-1.5">
						...
						<div className="relative">
							<MapPinIcon className="absolute end-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
							<Input
								id="location"
								value={location}
			...
			>
				<div className="grid gap-3">
					{[
						{ icon: <LinkIcon className="h-4 w-4" />, placeholder: "https://موقعك.com", value: website, onChange: (v: string) => setWebsite(v), label: "الموقع الشخصي" },
						{ icon: <TwitterLogoIcon className="h-4 w-4" weight="fill" />, placeholder: "@اسم_المستخدم", label: "تويتر / X" },
						{ icon: <InstagramLogoIcon className="h-4 w-4" weight="fill" />, placeholder: "@اسم_المستخدم", label: "إنستغرام" },
						{ icon: <GoodreadsLogoIcon className="h-4 w-4" weight="fill" />, placeholder: "رابط ملفك في Goodreads", label: "Goodreads" },
					].map((item, i) => (
			...
			<div className="flex items-center justify-between rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
				<p className="text-xs text-muted-foreground">
					التغييرات ستظهر فوراً في ملفك الشخصي
				</p>
