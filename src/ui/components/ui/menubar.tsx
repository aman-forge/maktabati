...
function Menubar({ className, ...props }: MenubarPrimitive.Props) {
  return (
    <MenubarPrimitive
      data-slot="menubar"
      className={cn("flex h-9 items-center rounded-2xl border p-1", className)}
      {...props}
    />
  );
}
...
function MenubarCheckboxItem({
  ...
  return (
    <MenuPrimitive.CheckboxItem
      ...
      {...props}
    >
      <span className="pointer-events-none absolute inset-s-3 flex size-4 items-center justify-center [&_svg:not([class*='size-'])]:size-4">
...
function MenubarLabel({
  ...
  return (
    <DropdownMenuLabel
      ...
      data-inset={inset}
      className={cn("px-3.5 py-2.5 text-xs text-muted-foreground data-inset:ps-9.5", className)}
      {...props}
    />
  );
}
